"""Social media generator for Aurora Corp — 11 tables across 6 platforms."""
...
SOCIAL_ACCOUNTS = [
    ("@auroracorp", "aurora_corp", "brand"),
    ("@aurora_novahome", "novahome", "product"),
    ("@pulseaudio_br", "pulseaudio", "product"),
    ("@guardian_iq", "guardianiq", "product"),
    ("@aurora_online", "aurora_online", "ecommerce"),
    ("@aurora_corp_pt", "aurora_pt", "regional"),
]
...
class SocialMediaGenerator(BaseGenerator):
    """Generates all 11 social media tables for Aurora Corp."""
    ...
    def _build_posts(
        ...
        for _ in range(n):
            ...
            hashtags = [f"#aurora{i}" for i in range(n_tags)]
            rows.append({
                "id": new_uuid(),
                "account_id": acc_id,
                "platform": platform,
                "post_type": post_type,
                "caption": f"Check out our latest product! #aurora #novahome #{platform}",
